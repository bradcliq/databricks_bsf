...
migration_df = spark.sql("""
    ...
    FROM raw_us_corporates.tbl_delta_runz_parser_results
...
print(f"Found {migration_df.count()} unique header mappings to migrate")
...
dealers_df = spark.sql("""
    SELECT email as from_email, firm, name as display_name
